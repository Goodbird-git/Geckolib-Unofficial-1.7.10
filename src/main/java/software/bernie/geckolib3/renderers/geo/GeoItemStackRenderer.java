...
public class GeoItemStackRenderer implements IGeoRenderer<AnimatableStackWrapper>, IItemRenderer {
    // Register a model fetcher for this renderer
    static {
        AnimationController.addModelFetcher((AnimationController.ModelFetcher<AnimatableStackWrapper>) object -> {
            if (object instanceof AnimatableStackWrapper) {
                IItemRenderer renderer = MinecraftForgeClient.getItemRenderer(((AnimatableStackWrapper)object).getStack(), ItemRenderType.INVENTORY);
                if (renderer instanceof GeoItemStackRenderer) {
                    return ((GeoItemStackRenderer) renderer).getGeoModelProvider();
                }
            }
            return null;
        });
...
//            double out=0;
        ...
        this.render(AnimatableStackWrapper.wrapItemStack(itemStack), itemStack);
        GL11.glPopMatrix();
    }
    ...
    public void render(AnimatableStackWrapper animatable, ItemStack itemStack) {
    ...
    public Integer getUniqueID(AnimatableStackWrapper animatable) {
        ItemStack currentItemStack = animatable.getStack();
